...
def braille(message):
    ...
    for letter in each_letter_list:  # 약자 적용
        if type(letter) == NormalLetter:
            ...
            for i in ['ㅅ', 'ㅈ', 'ㅊ', 'ㅆ', 'ㅉ']:
                abbreviation(i, 'ㅓ', 'ㅇ', '⠻')  # 성, 정, 청, 썽, 쩡은 기존 ᅟᅧᆼ의 약자를 사용하여 표기 (한·점 제16항)
            for i in range(len(abbr_list_jung_cjj)):
                abbreviation(abbr_list_cho_cjj[i], abbr_list_jung_cjj[i], abbr_list_jong_cjj[i], abbr_list_braille_cjj[i], 'cjj')  # '것'과 '껏'은 고유의 약자 사용 (한·점 제12항, 제14항)
